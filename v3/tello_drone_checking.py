    ...
    while True:
        ...
        if data:
            ...
            if data == "connect":
                ...
                info = {
                    "battery": tello_sim.get_battery(),
                    "altitude": tello_sim.altitude,
                    "flight_time": tello_sim.get_flight_time(),
                    "speed_x": tello_sim.get_speed_x(),
                    "speed_y": tello_sim.get_speed_y(),
                    "speed_z": tello_sim.get_speed_z(),
                    "pitch": tello_sim.get_pitch(),
                    "roll": tello_sim.get_roll(),
                    "state": "flying" if tello_sim.is_flying else "landed"
                }
                conn.send(str(info).encode())  # Send info back as a string
